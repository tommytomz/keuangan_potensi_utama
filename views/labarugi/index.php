...
$subtitle = '';
if(isset($_GET['Transaksi']['tanggal_dari'])){
    $date = explode('-', $_GET['Transaksi']['tanggal_dari']);
    $tahunawal = $date[2]-1;
    $tahunakhir = $date[2];
    $subtitle = $tahunawal." - ".$tahunakhir;
    //$subtitle = 'Tanggal: '.isset($_GET['Transaksi']['tanggal_dari']) ? $_GET['Transaksi']['tanggal_dari'].' s/d '.$_GET['Transaksi']['tanggal_sampai']:'';
}
?>
...
<div class="transaksi-index">
    <div id="ajaxCrudDatatable">
        <?=GridView::widget([
            'headertitle' => 'LAPORAN KEUANGAN FAKULTAS',
            'subtitle' => $subtitle,
            'id'=>'crud-datatable',
...
<?php
    $js = "
        jQuery('#caridata').click(function() {
            //alert('tes');
            $.pjax.reload({'push':true, 'replace':true, 'scrollTo':false, container: '#crud-datatable-pjax', url:'".Yii::$app->request->baseUrl."/index.php?r=labarugi&Transaksi[tanggal_dari]='+$( 'input#tanggal_dari' ).val()+'&Transaksi[tanggal_sampai]='+$( 'input#tanggal_sampai' ).val(), timeout: 1000});
        });
