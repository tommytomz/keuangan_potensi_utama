...
class PendapatanController extends Controller
{
    ...
    {    
        ...
        if(isset(Yii::$app->request->queryParams['PendapatanSearch']['idprodi'])){
            $idprodi = Yii::$app->request->queryParams['PendapatanSearch']['idprodi'];
        }

        if(isset(Yii::$app->request->queryParams['PendapatanSearch']['idfakultas'])){
            $idfakultas    = Yii::$app->request->queryParams['PendapatanSearch']['idfakultas'];
        }
        ...
        if($request->isAjax){
            ...
            */
            ...
            if($request->isGet){
                ...
                $ceksimpanmhs = 1;
                //echo $_POST['Pendapatan']['debet'];

                if($_POST['Pendapatan']['debet']=='2'){
                    foreach ($data as $key => $value) {
                        if($key>2){
                            $modelmahasiswa = new Mahasiswa();
                            //echo $searchModel->searchProdi(substr($value['E'],7,2))."<br>";
                            $kodeprodi[] = $searchModel->searchProdi(substr($value['E'],7,2));
                            $dataprodi[$searchModel->searchProdi(substr($value['E'],7,2))][] = trim($value['F']);

                            $modelmahasiswa->nim                = substr($value['E'],5,10);
                            $modelmahasiswa->nama               = $value['D'];
                            $modelmahasiswa->idprodi            = $searchModel->searchProdi(substr($value['E'],7,2));
                            $modelmahasiswa->angkatan           = substr($value['E'],5,2);
                            $modelmahasiswa->semester           = '';
                            $modelmahasiswa->tagihan            = trim($value['F']);
                            $modelmahasiswa->biaya_pendidikan   = 0;
                            $modelmahasiswa->total_bayar        = trim($value['F']);
                            $modelmahasiswa->tanggal_bayar      = "20".substr($value['E'],16,2)."-".substr($value['E'],18,2)."-".substr($value['E'],20,2);
                            $modelmahasiswa->keterangan         = "";
                            
                            if($modelmahasiswa->save()){
                                $ceksimpanmhs = 1;
                            }else{
                                $ceksimpanmhs = 0;
                            }
                ...
                if($_POST['Pendapatan']['debet']=='3'){
                    //print_r($data);
                    foreach ($data as $key => $value) {

                        $tagihan = explode(",", str_replace(".", "", trim($value['E'])));
                        //echo $tagihan[0];
                        if($key>1){
                            $modelmahasiswa = new Mahasiswa();
                            //echo $searchModel->searchProdi(substr($value['E'],7,2))."<br>";
                            $kodeprodi[] = $searchModel->searchProdi(substr($value['G'],2,2));
                            $dataprodi[$searchModel->searchProdi(substr($value['G'],2,2))][] = $tagihan[0];

                            $modelmahasiswa->nim                = trim($value['G']);
                            $modelmahasiswa->nama               = "";
                            $modelmahasiswa->idprodi            = $searchModel->searchProdi(substr($value['G'],2,2));
                            $modelmahasiswa->angkatan           = substr($value['G'],0,2);
                            $modelmahasiswa->semester           = '';
                            $modelmahasiswa->tagihan            = $tagihan[0];
                            $modelmahasiswa->biaya_pendidikan   = 0;
                            $modelmahasiswa->total_bayar        = $tagihan[0];
                            $modelmahasiswa->tanggal_bayar      = "";
                            $modelmahasiswa->keterangan         = "";
                ...
                $vdataprodi = array_unique($kodeprodi);
               // print_r($vdataprodi);
                $totaluangkuliah = 0;
                $ceksimpan = 0;
                if($ceksimpanmhs==1){
                    foreach ($vdataprodi as $key => $value) {
                        $model2 = new Pendapatan();
                        $model2->idprodi            = $value;
                        $model2->idsubakun          = $_POST['Pendapatan']['idsubakun'];
                        $model2->jumlah             = array_sum($dataprodi[$value]);
                        $model2->idtahunajaran      = $_SESSION['idtahunajaran'];
                        $totaluangkuliah            += $model2->jumlah ;
                        //echo $model2->jumlah."<br>";
                        if($model2->save()){
                            $ceksimpan = 1;
                        }
                    }
                ...
                if($ceksimpan==1){
                    ...
                    $modeltransaksi = new Transaksi();
                    $modeltransaksi->idsubakun       = $_POST['Pendapatan']['idsubakun'];
                    $modeltransaksi->idakundebet     = $_POST['Pendapatan']['idsubakun'];
                    $modeltransaksi->ke_akun         = $_POST['Pendapatan']['debet'];
                    $modeltransaksi->no_ref          = $noref;
                    $modeltransaksi->kredit          = $totaluangkuliah;
                    $modeltransaksi->keterangan      = 'Pembayaran dari Bank';
                    ...
                    if($modeltransaksi->save()){
                        ...
                        $modeltransaksi->idsubakun       = $_POST['Pendapatan']['debet'];
                        $modeltransaksi->idakunkredit    = $_POST['Pendapatan']['debet'];
                        $modeltransaksi->ke_akun         = $_POST['Pendapatan']['idsubakun'];
                        $modeltransaksi->no_ref          = $noref;
                        $modeltransaksi->debet           = $totaluangkuliah;
                        $modeltransaksi->keterangan      = 'Pembayaran dari Bank';
                        //$model->tanggal         = $_POST['Transaksi']['tanggal'];
                        $modeltransaksi->tanggal         = date('Y-m-d');
                        if($modeltransaksi->save()){
...
style="width:100%; height:500px;" frameborder="0"></iframe>',
    ...
    public function actionDelete($id)
    {
        $request = Yii::$app->request;
        // 
        $modelpendapatan    = Pendapatan::find()->where(['id'=>$id])->one();
        $modeltransaksi     = Transaksi::find()
            ->where(['no_ref'=>$modelpendapatan->no_ref, 'idsubakun'=>$modelpendapatan->idsubakun])
            ->one();
        $modeltransaksi->delete();
        $modeltransaksi     = Transaksi::find()
            ->where(['no_ref'=>$modelpendapatan->no_ref, 'ke_akun'=>$modelpendapatan->idsubakun])
            ->one();
        $modeltransaksi->delete();
